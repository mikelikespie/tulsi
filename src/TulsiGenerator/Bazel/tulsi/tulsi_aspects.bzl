...
def _cc_extract_generated_sources_and_includes(target):
  """Returns (source_metadatas, includes) generated by the given target."""
  file_metadatas = []
  includes = []
  cc_provider = _get_opt_attr(target, 'cc')
  if hasattr(cc_provider, 'transitive_headers'):
    file_metadatas = [_file_metadata(f) for f in cc_provider.transitive_headers]
...
def _module_name(ctx, label):
  return _get_opt_attr(ctx, 'rule.attr.module_name') or (
    label
      .replace("//", "")
      .replace("@", "")
      .replace("/", "_")
...
def _tulsi_sources_aspect(target, ctx):
  ...
  if target_kind in _SOURCE_GENERATING_RULES:
    ...
    generated_files += _cc_extract_generated_sources_and_includes(target)
  elif target_kind in _NON_ARC_SOURCE_GENERATING_RULES:
    generated_non_arc_files, generated_includes = (
        _extract_generated_sources_and_includes(target))
    generated_non_arc_files += _cc_extract_generated_sources_and_includes(target)
